...
class DistroDetector:
    ...
    def _get_forced_distro_info(self) -> Dict[str, str]:
        ...
        distro_id = self.force_distro.lower() if self.force_distro else ""
        ...
        if distro_id in known_distros:
            ...
            return {
                "id": distro_id,
                "name": self.force_distro or "unknown",
                "version": "unknown",
                "family": "unknown",
                ...
                try:
                    ...
                    distro_info = {
                        ...
                        "family": self._determine_family(
                            info.get("ID_LIKE", ""), info.get("ID", "")
                        ),
                        "package_manager": self._determine_package_manager(
                            info.get("ID", "")
                        ),
    ...
    def _detect_from_package_managers(self) -> Optional[Dict[str, str]]:
        ...
        for pm_cmd, (distro_id, name, family) in package_managers.items():
            try:
                subprocess.run(["which", pm_cmd], capture_output=True, check=True)
                ...
                self.logger.debug(
                    f"Detected from package manager {pm_cmd}: {distro_info}"
                )
                return distro_info
    ...
    def _get_service_management_info(
    ...
    def _get_network_management_info(
        self, distro_info: Dict[str, str]
    ) -> Dict[str, str]:
        ...
        if distro_info["family"] == "debian":
            ...
            return {"primary": "unknown", "restart_cmd": "systemctl restart networking"}

    def _get_firewall_management_info(
        self, distro_info: Dict[str, str]
    ) -> Dict[str, str]:
        ...
        if distro_info["family"] == "debian":
            ...
            return {"primary": "iptables", "reload_cmd": "iptables-restore"}

    def _get_package_management_info(
        self, distro_info: Dict[str, str]
    ) -> Dict[str, str]:
