...
class RevertITDaemon:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "distro": {"auto_detect": True, "force_distro": None},
        }
    ...
    def initialize_components(self) -> None:
        ...
        self.snapshot_manager = SnapshotManager(
            config=self.config["snapshot"], distro_info=distro_info
        )
        ...
        self.timeout_manager = TimeoutManager(
            config=self.config["timeout"], revert_engine=self.revert_engine
        )
    ...
    def start_monitoring_threads(self) -> None:
        ...
        if self.config_monitor:
            self.monitor_thread = threading.Thread(
                target=self.config_monitor.start_monitoring, daemon=True
            )
        ...
        if self.timeout_manager:
            self.timeout_thread = threading.Thread(
                target=self.timeout_manager.start_processing, daemon=True
            )
...
def main():
    ...
    parser.add_argument(
        "--config", default="/etc/revertit/config.yaml", help="Configuration file path"
    )
    parser.add_argument(
    ...
    revertit_daemon = RevertITDaemon(config_path=args.config)
