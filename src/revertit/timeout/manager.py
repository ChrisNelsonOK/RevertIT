...
class TimeoutManager:
    ...
    def __init__(self, config: Dict[str, Any], revert_engine):
        ...
        self.connectivity_endpoints = config.get(
    ...
    def start_timeout(
        ...
        with self.lock:
            ...
            existing_entries = [
                entry_id
                for entry_id, entry in self.active_timeouts.items()
                if entry.file_path == file_path
            ]

            for entry_id in existing_entries:
                self.logger.info(
                    f"Cancelling existing timeout for {file_path}: {entry_id}"
                )
        ...
        self.logger.info(
            f"Started timeout for {change_category} change: {file_path} "
    ...
    def confirm_change(self, change_id: str) -> bool:
        ...
        self.logger.info(
            f"Configuration change confirmed: {change_id} ({timeout_entry.file_path})"
        )
                ...
                timeouts.append(
                    {
                        ...
                        "remaining_seconds": max(
                            0, int(remaining_time.total_seconds())
                        ),
    ...
    def _handle_timeout_expiry(self, change_id: str) -> None:
        ...
        self.logger.warning(
            f"Configuration change timeout expired: {change_id} ({entry.file_path})"
        )
        ...
        if self.connectivity_check:
            if self._check_connectivity():
                self.logger.info(
                    "Connectivity check passed, but timeout expired - proceeding with revert"
                )
            else:
                self.logger.warning(
                    "Connectivity check failed - configuration change may have broken connectivity"
                )

        # Grace period before revert
        if self.grace_period > 0:
            self.logger.info(
                f"Grace period of {self.grace_period} seconds before revert"
            )
    ...
    def _format_notification_message(self, entry: TimeoutEntry, event_type: str) -> str:
        ...
        return messages.get(
            event_type, f"Timeout event ({event_type}): {entry.file_path}"
        )
