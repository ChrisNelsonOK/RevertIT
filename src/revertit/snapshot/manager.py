...
class SnapshotManager:
    ...
    def __init__(self, config: Dict[str, Any], distro_info: Dict[str, str]):
        ...
        self.snapshot_location = Path(
            config.get("snapshot_location", "/var/lib/revertit/snapshots")
        )
        ...
        self.logger.info(
            f"Snapshot manager initialized. TimeShift available: {self.timeshift_available}"
        )
    ...
    def _check_timeshift_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["which", "timeshift"], capture_output=True, text=True, check=False
            )
    ...
    def _backup_path(
        self, source_path: str, snapshot_dir: Path, metadata: Dict
    ) -> None:
    ...
    def _list_timeshift_snapshots(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = subprocess.run(
                ["timeshift", "--list"], capture_output=True, text=True, check=True
            )
                        ...
                        snapshots.append(
                            {
                                ...
                                "timestamp": parts[
                                    1
                                ],  # TimeShift uses timestamp as name
                                "description": (
            ...
            if item.is_dir() or item.name.endswith(".tar.gz"):
                ...
                if metadata_file and metadata_file.exists():
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to load metadata for {snapshot_id}: {e}"
                        )
    ...
    def _delete_timeshift_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to delete TimeShift snapshot {snapshot_id}: {e.stderr}"
            )
            return False
