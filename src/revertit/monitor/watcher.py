...
class ConfigurationMonitor:
    ...
    def __init__(self, config: Dict[str, Any], timeout_manager, snapshot_manager):
        ...
        self.logger.info(
            f"Configuration monitor initialized with {len(self.monitor_paths)} paths"
        )
    ...
    def _expand_glob_pattern(self, pattern: str) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(
                f"Glob pattern '{pattern}' expanded to {len(existing_matches)} files"
            )
        ...
        for directory, files in directories_to_watch.items():
            if os.path.exists(directory):
                try:
                    ...
                    self.logger.debug(
                        f"Watching directory: {directory} (files: {len(files)})"
                    )
                except Exception as e:
    ...
    def _categorize_change(self, file_path: str) -> str:
        ...
        if any(
            net_path in path_lower
            for net_path in ["network", "netplan", "networkmanager", "interfaces"]
        ):
            return "network"
        ...
        if any(
            fw_path in path_lower
            for fw_path in ["iptables", "ufw", "firewall", "firewalld"]
        ):
            return "firewall"
...
class ConfigurationEventHandler(FileSystemEventHandler):
    ...
    def __init__(
        self, monitored_files: Set[str], on_change_callback: Callable[[str, str], None]
    ):
    ...
    def on_moved(self, event):
        ...
        for file_path, event_type in [
            (event.src_path, "moved_from"),
            (event.dest_path, "moved_to"),
        ]:
            if self._is_monitored_file(file_path) and self._should_process_event(
                file_path
            ):
